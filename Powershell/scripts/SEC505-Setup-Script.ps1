...
#  You might have the CD as an ISO file.  Copy the ISO contents, not the file.
#  Run PowerShell as administrator (right-click and Run ISE As Administrator).
...
if (-not (test-path C:\SANS)) 
{
    ...
    " administrative privileges. Your course CD-ROM may have been provided"
	" to you as an ISO file on an USB flash drive.`n`n"
    exit
}
...
copy-item C:\SANS\Day1-PowerShell\Compare-FileHashesCSV.ps1 C:\Temp -Force
