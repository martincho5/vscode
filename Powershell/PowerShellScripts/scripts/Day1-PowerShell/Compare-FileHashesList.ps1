...
[CmdletBinding()]
Param ( $ReferenceFile, $DifferenceFile, [Switch] $IncludeUnchanged, [Switch] $SummaryOnly ) 
...
if ($IncludeUnchanged) { $samehashtable = New-TypedDictionary -KeyType 'String' -ValueType 'String' -InitialSize $InitialSize } 
...
else #Assume a non-CSV file was created with one of the *DEEP tools (or anything with the same output format as MD5DEEP)
{
    ...
    {
        if ($line.Length -le 31 -or $line.IndexOf('#') -eq 0){ Continue }  #Ignore headers
        $line = $line -replace '(^[0-9a-f]{32,})\W{2,2}(.+$)','$1;$2'
        $semi = $line.IndexOf(';') 
        $path = $line.Substring($semi + 1) 
        $hash = $line.Substring(0,($line.length - $path.length - 1))  
...
{ 
    ...
    { 
        ...
        #Remember, hashtables with file paths as unique keys and hashes as their values, so:            
